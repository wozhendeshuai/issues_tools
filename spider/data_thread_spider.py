...
def create_table(table_name):
    ...
    if table_exists:
        ...
        execute_query( f"CREATE TABLE {table_name} (issues_number INT AUTO_INCREMENT PRIMARY KEY, issues_text LONGTEXT, labels VARCHAR(255))")
...
def batch_set_issues_info(db_max_number, max_issues_number, owner_name, repo_name):
    ...
    while page_num <= max_page:
        try:
            print("==="+threading.current_thread().name+"==================开始获取第" + str(page_num) + "页issues================================")
        ...
        if list_issues_request.status_code >= 200 and list_issues_request.status_code < 300:
            ...
            for issues_index in range(0, length_list_issues_json):
                print("====="+threading.current_thread().name+"===========开始获取第" + str(page_num) + "页,第" + str(issues_index) + "个issues===========")
                # 从json中提取数据
