...
logging.log(level=logging.DEBUG, msg=time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))


def log_str(method_name, owner_name, repo_name, table_name, max_issues_number, db_max_number, message):
    reStr = "  线程名:" + threading.current_thread().name
...
def create_table(table_name):
    ...
    if table_exists:
        logging.log(level=logging.DEBUG,
                    msg=("create_table", None, None, table_name, None, None, "名为：" + table_name + "的表已存在"))
    else:
        execute_query(
            f"CREATE TABLE {table_name} (issues_number INT ,owner_repo_name VARCHAR(255), issues_text LONGTEXT, labels VARCHAR(255))")
        logging.log(level=logging.DEBUG, msg=log_str("create_table", None, None, table_name, None, None,
                                                     "名为：" + table_name + "的表创建成功"))
...
def write_to_table(table_name, owner_name, repo_name, issues_number, issues_text, labels):
    ...
    logging.log(level=logging.DEBUG, msg=log_str("write_to_table", None, repo_name, table_name, None, None,
                                                 "在表中" + table_name + "插入" + str(
                                                     issues_number) + "的issues_text和labels"))


# 封装成一个方法，让他方便外部调用
def batch_set_issues_info(db_max_number, max_issues_number, owner_name, repo_name, table_name):
    # 获取当前页面
    db_Page_number = int(db_max_number / 100) + 1
    # repo url拼接
    ...
    for page_num in tqdm.tqdm(iterable=range(db_Page_number, max_page),
                              desc="正在获取" + owner_name + "_" + repo_name + "的issues",
                              ):
        try:
            logging.log(level=logging.DEBUG,
                        msg=log_str("batch_set_issues_info", owner_name, repo_name, table_name, max_issues_number,
                                    db_max_number,
                                    "开始获取第" + str(page_num) + "页issues"))
            list_issues_request = requests.get(list_issues_url, headers=headers)
            logging.log(level=logging.DEBUG,
                        msg=log_str("batch_set_issues_info", owner_name, repo_name, table_name, max_issues_number,
                                    db_max_number, "获取第" + str(page_num) + "页issues成功"))
        ...
        if list_issues_request.status_code >= 200 and list_issues_request.status_code < 300:
            ...
            # print(log_str("batch_set_issues_info", owner_name, repo_name, table_name, max_issues_number, db_max_number,"length_list_issues_json:" + str(length_list_issues_json)))
            for issues_index in range(0, length_list_issues_json):
                # print(log_str("batch_set_issues_info", owner_name, repo_name, table_name, max_issues_number,db_max_number, "开始获取第" + str(page_num) + "页,第" + str(issues_index) + "个issues"))

                # 如果这个json包含pull_request字段，则说明这个issues是PR，跳过
                if list_issues_json[issues_index].__contains__("pull_request"):
                    continue
                # 从json中提取数据
                ...
                if issues_number <= db_max_number:
                    logging.log(level=logging.DEBUG,
                                msg=log_str("batch_set_issues_info", owner_name, repo_name, table_name,
                                            max_issues_number, db_max_number,
                                            "这个issues已经爬取过了，跳过，issues_number:" + str(issues_number)))
                    continue
                ...
                logging.log(level=logging.DEBUG,
                            msg=log_str("batch_set_issues_info", owner_name, repo_name, table_name, None, None,
            ...
            logging.log(level=logging.DEBUG,
                        msg=log_str("batch_set_issues_info", owner_name, repo_name, table_name, max_issues_number,
                                    ...
                                    "list_issues_url: " + list_issues_url + "  Status Code:" + str(
                                        list_issues_request.status_code) + "  Error Response:" + str(
                                        list_issues_request.text)))
            break
...
def get_closed_issues_max_num(owner_name, repo_name):
    ...
    logging.log(level=logging.DEBUG,
                msg=log_str("get_closed_issues_max_num", owner_name, repo_name, "closed_issues_number_tbl", None, None,
    ...
    try:
        list_issues_request = requests.get(list_issues_url, headers=headers)
        logging.log(level=logging.DEBUG,
                    msg=log_str("get_closed_issues_max_num", owner_name, repo_name, "closed_issues_number_tbl", None,
                                None,
                                " list_issues_url: " + list_issues_url + " Status Code:" + str(
                                    list_issues_request.status_code)))
    except Exception as e:
        # 如果发生错误则回滚
        logging.log(level=logging.DEBUG,
                    msg=log_str("get_closed_issues_max_num", owner_name, repo_name, "closed_issues_number_tbl", None,
    ...
    if list_issues_request.status_code >= 200 and list_issues_request.status_code < 300:
        ...
        if issues_number <= closed_issues_number:
            ...
            if len(result) == 0:
                ...
                logging.log(level=logging.DEBUG,
                            msg=log_str("get_closed_issues_max_num", owner_name, repo_name, "closed_issues_number_tbl",
                                        closed_issues_number, None,
                                        "插入的closed_issues_number :" + str(closed_issues_number)))
            else:
...
def process(owner_name, repo_name):
    ...
    result = execute_select_query(
        f"SELECT MAX(issues_number) FROM {table_name} where owner_repo_name='{owner_name + '_' + repo_name}'")
    if result[0][0] is None:
        db_max_number = 0
    else:
        db_max_number = int(result[0][0])
    logging.log(level=logging.DEBUG,
                msg=log_str("process", owner_name, repo_name, table_name, max_issues_number, db_max_number, None))
